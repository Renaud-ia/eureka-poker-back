...
public class ParserIPoker extends ParserXml {
    private String nomHero;
    private final ExtracteurBetclic extracteurBetclic;
    private final List<CartePoker> cartesBoard;
    public ParserIPoker(ObservateurParser observateurParser, Document document, ExtracteurBetclic extracteurBetclic) {
    ...
    private void extraireTours(Element mainElement) throws ErreurLectureFichier, JoueurNonExistant {
        ...
        for (int i = 0; i < tourElements.getLength(); i++) {
            ...
            if (!tourElement.hasAttribute("no")) continue;
    ...
    private void extraireBlindesAnte(Element tourElement) throws ErreurLectureFichier {
        ...
        BigDecimal montantBB = new BigDecimal(0);
        ...
        for (int i = 0; i < actionsElements.getLength(); i++) {
            ...
            if (blindeOuAnte.isBlinde()) {
                ...
                if (blindeOuAnte.obtMontant().compareTo(montantBB) > 0) montantBB = blindeOuAnte.obtMontant();
            }
